...
/**
 * Bean implementation class of the EncryptionEJB. Shows how lifecycle callbacks are implemented (@PostConstruct), and two ways of obtaining
 * externalized environment entries.
 *
 */
...
public class EncryptionBean implements EncryptionLocalBusiness, EncryptionRemoteBusiness {
    ...
    /**
     * Name we'll assign to this EJB, will be referenced in the corresponding META-INF/ejb-jar.xml file
     */
    static final String EJB_NAME = "EncryptionEJB";

    /**
     * Name of the environment entry representing the ciphers' passphrase supplied in ejb-jar.xml
     */
    private static final String ENV_ENTRY_NAME_CIPHERS_PASSPHRASE = "ciphersPassphrase";
    /**
     * Name of the environment entry representing the message digest algorithm supplied in ejb-jar.xml
     */
...
* and not "conversational state".
     */
    /**
     * SessionContext of this EJB; this will be injected by the EJB Container because it's marked w/ @Resource
     */
    @Resource
    private SessionContext context;
    /**
     * Passphrase to use for the key in cipher operations; lazily initialized and loaded via SessionContext.lookup
     */
    private String ciphersPassphrase;
    /**
     * Algorithm to use in message digest (hash) operations, injected via @Resource annotation with name property equal to env-entry name
     */
     ...
     */
    @Override
    public String decrypt(final String input) throws IllegalArgumentException, IllegalStateException, EncryptionException {
        // Get the cipher
        final Cipher cipher = this.decryptionCipher;
        if (cipher == null) {
            throw new IllegalStateException("Decryption cipher not available, hasthis service been initialized ?");
        }
        // Run the cipher
     ...
     * Note:
     *
     * This is a weak implementation, but is enough to satisfy the example.* If considering real-world stresses, we would be, at a minimum:
     *
     * 1) Incorporating a random salt and storing it alongside the hashed result 2) Additionally implementing an iteration count to re-hash
     * N times
     */
    @Override
    ...
    /**
     * Override the way we get the ciphers' passphrase so that we may define it in a secure location on the server. Now our production
     * systems will use a different key for encoding than our development servers, and we may limit the likelihood of a security breach
     * while still allowing our programmer to use the default passphrase transparently during development.
     *
     * If not provided as an env-entry, fall back upon the default.
     *
     * Note that a real system won't expose this method in the public API, ever. We do here for testing and to illustrate the example.
     *
     * @see org.jboss.ejb3.examples.ch05.encryption.EncryptionBeanBase#getCiphers Passphrase()
     */
    @Override
    ...
    /**
     * Obtains the message digest algorithm as injected from the env-entry element defined in ejb-jar.xml. If not specified, fall back onto
     * the default, logging a warn message
     *
     * @see org.jboss.ejb3.examples.ch05.encryption.EncryptionRemoteBusiness#get MessageDigestAlgorithm()
     */
    @Override
...
// ---------------------------------------------------------------------------||
    ...
    /**
     * Obtains the environment entry with the specified name, casting to a String, and returning the result. If the entry is not assignable
     * to a String, an {@link IllegalStateException} will be raised. In the event that the specified environment entry cannot be found, a
     * warning message will be logged and we'll return null.
     *
     * @param envEntryName
     * @return
    ...
    private String getEnvironmentEntryAsString(final String envEntryName) throws IllegalStateException {
        ...
        if (context == null) {
            log.warn(
                    "No SessionContext, bypassing request to obtain environment entry: " + envEntryName);
            return null;
        }
    ...
    /**
     * Returns a String representation of the specified byte array using the charset from {@link EncryptionBeanBase#getCharset()}. Wraps any
     * {@link UnsupportedEncodingException} as a result of using an invalid charset in a {@link RuntimeException}.
     *
     * @param bytes
     * @return
     * @throws RuntimeException If the charset was invalid, or some other unknown error occurred
    ...
    /**
     * Returns a byte array representation of the specified String using the charset from {@link EncryptionBeanBase#getCharset()}. Wraps any
     * {@link UnsupportedEncodingException} as a result of using an invalid charset in a {@link RuntimeException}.
     *
     * @param input
     * @return
    ...
    /**
     * Obtains the charset used in encoding/decoding Strings to/from byte representation
     *
     * @return The charset
     */
